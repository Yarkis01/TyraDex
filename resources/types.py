...
class Types(Resource):
    def get(
        self, first_type: Union[str, int] = None, second_type: Union[str, int] = None
    ) -> dict:
        if not first_type:
            return JSON_types
        ...
        first_type_id = (
            int(first_type)
            if first_type.isdigit()
            else JSON_types_to_id.get(str(first_type).lower(), -1)
        )
        if first_type_id == -1:
        ...
        if second_type:
            second_type_id = (
                int(second_type)
                if second_type.isdigit()
                else JSON_types_to_id.get(str(second_type).lower(), -1)
            )
            if second_type_id == -1:
        ...
        if not second_type or first_type_id == second_type_id:
            ...
            json["pokemons"] = [
                ...
                and (
                    ...
                    or (
                        len(pokemon["types"]) > 1
                        and str(pokemon["types"][1]["name"]).lower()
                        == JSON_id_to_types[first_type_id]
                    )
                )
            ]
            ...
            "pokemons": [
                ...
                and (
                    (
                        ...
                        == JSON_id_to_types[first_type_id]
                        and str(pokemon["types"][1]["name"]).lower()
                        == JSON_id_to_types[second_type_id]
                    )
                    or (
                        str(pokemon["types"][0]["name"]).lower()
                        == JSON_id_to_types[second_type_id]
